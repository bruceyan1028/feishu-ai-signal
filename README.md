...
标题、链接、来源、`source_id`、发布时间、原文、中文正文、正文排版、中文标题、中文摘要、内容分类、AI深度解读、为何重要、主题、影响分 / 新颖度 / 可行动性 / 紧迫度、状态（待分析/已分析）、去重键、媒体资源（JSON）、质量分、论文/社媒/播客指标。

`daily` 分析后回写中文标题、摘要、解读和分数。简报 JSON 由这些字段组装，不再依赖分析时的飞书原文。

参数表的 `dimension=中文媒体` 只描述来源属性，不作为简报主题。中文媒体条目由 LLM
另写 `内容分类`，从前沿模型公司、技术研究开源、算力芯片云、政策监管地缘、
模型评测基准、产品化企业采用、创业融资并购、其他中选择；网页筛选不展示
「中文媒体」主题标签。
...
例外：分类为「中文媒体」或载体为「公众号」的中文稿走 `daily.verbatim_body_mode`，详情页展示原文而不是 `deepAnalysis`（连 `##` / `###` / `####` 各级小标题一起保留）。原文抓得太短（<200 字）或其实是英文时自动退回深度解读那条线。

上屏前先过一遍 `daily.polish_verbatim_body`：抓下来的中文稿尾部普遍挂着版权、举报、联系方式和账号名片，小标题又常被并进段首，模型负责删掉这些页面模板并把小标题切回独立的 `##` 行。这一步只许删减与分段，`daily.verbatim_fidelity` 逐句核对，改写比例超标就整片退回未清理的原文。结果按 `正文排版` / `正文排版版本` 缓存，同一条不会每天重跑；调整提示词后把 `READABLE_BODY_VERSION` +1 即可让存量条目重刷。片段大小与处理上限见 `BODY_POLISH_*`。
...
- **HF 三栏照官网的卡片排法，不套 AA 那种「一行 + 进度条」**：这些数字正是判断「值不值得点」的依据，塞进 tooltip 等于没给。官网不显示名次也不显示热度分，这里也不加——顺序本身就是「本周热度」。
  - **每条是独立的浅底卡片**（`hf-item`：`--ds-card` 白底 + `--ds-border` 描边 + 6px 空隙）：空隙里透出看板的黑就是分隔，比在黑底上叠一层更亮的黑分得清得多，官网首页也是这么排的。颜色一律取 `:root` 那套浅色 token——整页本来就是白底，只有右栏是显式刷黑的，这里不另造一套；换成浅卡后字色也得跟着换（`--ds-text` / `--ds-text-2` / `--ds-text-3`），运行中的绿点取 `--ds-good` 而不是深色底那支亮绿（`#34d399` 铺在浅卡上会发灰）。悬停沿用信号卡那套硬阴影而不动底色：`accent-faint` 这类半透明叠加铺在浅卡上会把背后的黑透出来。直角，和右栏其余面板一致。
...
- 取 logo 只认厂商自己发布的文件：Qwen 用 chat.qwen.ai 的 `qwen-logo.svg`（蓝色风车 `#082DFF`，早先那版紫色三角是手绘的，形状和颜色都不对），Kimi 用官方 Branding-Guide 的 `k-only`，IBM 用官网 masthead 里的 8-bar，Liquid AI / OpenBMB 用各自站点的 SVG，Microsoft 四方块按官方色值摆。抽符号时用 Chrome 的 `getBBox()` 算包围盒再缩放到 24×24，不手量坐标。
- 没有官网的实验室（AI9Stars、Nanbeige 都只在 GitHub / Hugging Face 发模型）也没有域名可做 key，前端用 `LOGO_BY_CREATOR` 按厂商名认，`_creator_domain` 照实返回空串——**别编一个解析不了的域名当 key**。Nanbeige 例外：`nanbeige.com` 是他们指向自家 HF 组织的真实域名，仍走域名映射。
- Nanbeige 的 logo 取自家 HF 仓库里的 `figures/nbg.png`（无损 PNG，团队自己放的；HF 头像 CDN 只给有损 WebP，别用那个）。AI9Stars 全网查不到自建站，`.ai/.com/.cn/.org/.net` 都没解析，模型卡和仓库里也没有 logo，唯一自发布的是 GitHub 组织头像——但那版塞了闪电、字母和星点，18px 下糊成一团，所以用 AA 站上那份紫色矢量图，**这一个是权宜之计，不是厂商自发布来源**。
- 真查不到可信 logo 就留字标，**不要拿母公司 logo 或第三方图凑**——字标是「没查到」，错图是「查错了」。
