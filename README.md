...
参数表，并同步信号源表的「自动化状态」。采集本身仍然跑在 GitHub Actions 上，配置台不
参与定时任务。新增的源一律先落 `experimental`，诊断通过后再在页面上打开开关。

只读站和配置台都有搜索框，按名称、`source_id`、采集地址匹配，与「内容分类 / 信息类型」
两级筛选叠加生效。

配置台里每行左侧是采集开关：开 = `active`，关 = `paused`。关掉之后下一轮流水线不再抓
